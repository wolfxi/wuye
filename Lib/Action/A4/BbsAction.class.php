...
class BbsAction extends CommonAction {
	...
	public function _initialize(){
		...
		$this->_forum_name=$forum_list['WY_BBS'];


		//当前所在分类
		$navid=$_GET['navid'] ? $_GET["navid"] : $_POST["navid"];
		if(empty($navid)){
			$navid=$this->forumlist["WY_BBS"];
		}
	...
	public function create_message_type(){
		...
		$data['is_anon']=$_POST['anon'] ? 1 : 0;
		$data['is_bind']=$_POST['bind'] ? 1 : 0;
		if(!empty($data['name'])){
	...
	public function one_messagetype_list(){
		...
		if(!empty($kind_id)){
			...
			if(isset($_POST['search'])){
				...
				$where['title']=$_POST['title'] ? array("like","%".$_POST['title']."%") : null;
				...
				//$this->assign("slasttime",$_POST['lasttime']);
				//$this->assign("sfirsttime",$_POST['firsttime']);
			}else{
				$where['aid']=$kind_id;
				$where['is_delete']=0;
			}
			...
			$list = $me->where($where)->order('sort')->limit($Page->firstRow.','.$Page->listRows)->select();
	...
	public function update_messagetype(){
		...
		$data['is_anon']=isset($_POST['anon']) ? 1 : 0;
		$data['is_bind']=isset($_POST['bind']) ? 1 : 0;
		if(!empty($data['name'])){
			$mt=M("message_type");
	...
	public function search(){
		...
		if(!empty($searchstr)){
			...
			//获取该平台所有该模块下的类型编号
			$mt=M("message_type");
			$mtresult=$mt->where("xiaoqu_id = %d AND aflag ='%s'  AND is_delete =%d",$this->xiaoqu_id,$this->_forum_name,0)->order("sort")->getField("id");
			//查找信息
			$me=M("message");
			$where['is_delete']=array('eq',0);
