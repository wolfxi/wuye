...
class WyadsAction extends CommonAction {

	private $forumlist=null;

	private $daili_id=null;
	...
	Public function index(){
		$mt=M("message_type");
		$result=$mt->where("daili_id = %d AND aflag ='%s' AND is_delete = %d ",$this->daili_id,$this->forumlist['WY_ADS'],0)->order("sort , id desc")->select();
	...
	public function create_message_type(){
		...
		$data['is_diapley']=$_POST['display'] ? 1 : 0;//是否启用图标
		if(!empty($data['name'])){
			$data['daili_id']=$this->daili_id;
			$data['aflag']=$this->forumlist['WY_ADS'];
			$data['sort']=1000;
			$mt=M("message_type");
	...
	public function update_messagetype(){
		...
		$data['is_diapley']=$_POST['display'] ? 1 : 0;
		if(!empty($data['name'])){
			$mt=M("message_type");
	...
	public function move_message_type(){
		$moveid=$_POST["id"];
		$action=$_POST["action"];
		$where['aflag']=array("EQ",$this->forumlist['WY_ADS']);
		$where['daili_id']=array("EQ",$this->daili_id);
		$move=new MoveAction();
