...
class SelfformAction extends CommonAction {
    ...
    public function index(){
    	...
    	$result=$form->where("xiaoqu_id='%s' AND   aflag = '%s'",$id,$this->forumlist['WY_FORM'])->order('sort,id desc')->limit($Page->firstRow.','.$Page->listRows)->getField('id,name,keyworks,is_status,introduce');
    ...
    public function form_data_save(){
        ...
        //$this->error("请填写详细详细");
            ...
            if($result){
                ...
                //$this->error("编辑失败");
                $this->ajaxReturn(array("msg"=>"编辑成功"),"JSON");
            }
        }
    }
    ...
    public function elsemodellist(){
        ...
        $modelist=M("Message");
        $tit=M("Message_type");
        $titl==$tit->where("id='{$id}'")->find();

        $data['aid']=$id;
        $data['is_delete']=0;
       if($_POST['aid']){
            $aid=$_POST['aid'];
            $data['aid']=array("eq","{$aid}");
        }
...
/*
    //搜索模型数据
    public function sea_model(){
        $ms=M("Message");
        $where="";

        $account=trim($_POST['wxname']);
        $handle_status=trim($_POST['type']);
        $assess=trim($_POST['dp']);
        if(!empty($data['account'])){
            $where.="account='{$account}'";
        }else if($handle_status!=-1){
            $where.=" AND handle_status='{$handle_status}'";
        }else if($assess!=-1){
            $where.=" AND assess='{$assess}' AND is_delete=0";
        }
        import('ORG.Util.Page');// 导入分页类
        $count= $ms->where($where)->count();// 查询满足要求的总记录数
        $Page= new Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数
        $show= $Page->show();// 分页显示输出
        $this->assign('page',$show);// 赋值分页输出
        $result=$ms->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();

        $this->assign("result",$result);
        $this->display("elsemodellist");
